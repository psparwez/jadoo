...
<template>
  <header
    :class="{
      'relative z-40 bg-white/20 backdrop-blur-md border-b border-white/50  w-full pb-4':
        isNotFoundPage,
    }"
  >
    <div
      class="flex nav__container text-text-foreground justify-between items-center relative px-4 !pb-0 !pt-8 z-40"
    >
      <!-- Logo -->
      <div class="text-foreground-dark">
        <RouterLink to="/" class="text-[1.75rem] font-semibold text-foreground-dark">
          <!-- Jad<span class="text-primary">oo</span>. -->
          <svg
      ...
      <!-- Nav links -->
      <nav class="hidden md:flex gap-6 md:gap-10 items-center">
        <ul class="hidden lg:flex gap-6 md:gap-8 lg:gap-12 font-medium">
          <li v-for="(item, index) in navLinks" :key="index">
            <RouterLink
              :to="item.path"
              class="hover:text-primary duration-300 transition-colors !cursor-pointer"
        ...
        <div class="flex gap-5 items-center font-medium">
          <button class="hover:text-primary">Login</button>
          <button
            class="px-4 h-9 hover:bg-primary hover:text-white hover:outline-none inline-flex items-center gap-1 outline-1 outline-border/30 rounded-xl text-sm"
          >
            Sign Up
          </button>

          <select class="font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 px-2">
      ...
      <button
        @click="toggleNavbar"
        class="flex md:hidden size-10 rounded-full bg-white items-center justify-center shadow-2xl border-2 border-white active:opacity-95 active:scale-95 transition-all durations-200"
      >
        <transition name="icon-slide-fade">
          <svg
      ...
      <transition name="mobile-nav">
        <div
          v-if="isNavbarVisible"
          class="fixed block md:hidden left-0 top-0 min-h-screen w-[80%] sm:w-[60%] p-6 bg-white space-y-6 pt-9 overflow-y-auto"
        >
          <!-- Logo -->
          <div class="text-foreground-dark border-b pb-6 border-zinc-200">
            <RouterLink to="/" class="text-[1.75rem] font-semibold text-foreground-dark">
              <!-- Jad<span class="text-primary">oo</span>. -->
              <svg
                xmlns="http://www.w3.org/2000/svg"
          ...
          <nav class="flex flex-col gap-6 md:gap-10">
            <ul
              class="flex border-b pb-8 border-zinc-200 flex-col gap-6 md:gap-8 lg:gap-12 font-medium"
            >
              <li v-for="(item, index) in navLinks" :key="index">
                <RouterLink
                  :to="item.path"
            ...
            <div class="flex gap-4 flex-col font-medium">
              <button class="w-full bg-primary text-white p-3 rounded-2xl">Login</button>
              <button
                class="p-3 hover:bg-primary hover:text-white hover:outline-none inline-flex justify-center items-center gap-1 outline-1 outline-border/30 rounded-xl text-sm"
              >
                Sign Up
              </button>
            </div>
